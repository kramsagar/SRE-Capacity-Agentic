...
async def run(
    ...
    async with MCPClient() as mcp:
        ...
        # Load HPA name mapping from namespace_inventory.yaml
        import yaml as _yaml
        _inv      = _yaml.safe_load((ROOT / "references" / "namespace_inventory.yaml").read_text())
        _svc_list = _inv.get("namespaces", {}).get(namespace, {}).get("microservices", [])
        hpa_name_map = {}
        for _e in _svc_list:
            if isinstance(_e, dict):
                hpa_name_map[_e.get("name", "")] = _e.get("hpa_name", _e.get("name", ""))
            else:
                hpa_name_map[str(_e)] = str(_e)
        log.info(f"  HPA name map (YAML): {hpa_name_map}")
        log.info(f"  HPA keys (OCP):      {list(ocp_data['hpa'].keys())}")
        ...
        for svc in target_svcs:
            ...
            # Look up HPA by mapped name first, then partial match
            hpa_lookup = hpa_name_map.get(svc, svc)
            hpa_obj    = ocp_data["hpa"].get(hpa_lookup)
            if not hpa_obj:
                svc_clean = svc.replace("-", "").lower()
                for hpa_name, hpa_val in ocp_data["hpa"].items():
                    if svc_clean in hpa_name.replace("-", "").lower():
                        hpa_obj = hpa_val
                        log.info(f"  HPA partial match: {svc} → {hpa_name}")
                        break

...
async def check_grafana(
    ...
    async with httpx.AsyncClient(timeout=timeout, verify=False) as client:
        ...
        # 3 — /api/datasources (find the right Prometheus)
        ds_uid  = ""
        ds_name = ""
        ds_name_hint = os.getenv("GRAFANA_DS_NAME", "").strip()
        ds_uid_hint  = os.getenv("GRAFANA_DS_UID",  "").strip()
        try:
            ...
            if r.status_code == 200:
                ...
                chosen = None
                # Priority 1: exact name from GRAFANA_DS_NAME
                if ds_name_hint:
                    for ds in prom_sources:
                        if ds.get("name", "").strip() == ds_name_hint:
                            chosen = ds
                            break
                # Priority 2: uid from GRAFANA_DS_UID
                if not chosen and ds_uid_hint:
                    for ds in prom_sources:
                        if ds.get("uid") == ds_uid_hint:
                            chosen = ds
                            break
                # Priority 3: first
                if not chosen and prom_sources:
                    chosen = prom_sources[0]

                if chosen:
                    ds_uid  = chosen.get("uid",  "")
                    ds_name = chosen.get("name", "")
                    ds_url  = chosen.get("url",  "")
                    match_method = (
                        "GRAFANA_DS_NAME match" if ds_name_hint and ds_name == ds_name_hint
                        else "GRAFANA_DS_UID match" if ds_uid_hint and ds_uid == ds_uid_hint
                        else "first available"
                    )
                    results.append(CheckResult(
                        "Grafana: Prometheus datasource", "PASS",
                        f"name='{ds_name}'  uid={ds_uid}  [{match_method}]"
                    ))
                    log.info(
                        f"[Health] All Prometheus sources: "
                        f"{[(d.get('name'), d.get('uid')) for d in prom_sources]}"
                    )
                    log.info(
                        f"[Health] To pin this datasource add to .env:\n"
                        f"  GRAFANA_DS_NAME={ds_name}\n"
                        f"  GRAFANA_DS_UID={ds_uid}"
                    )
                else:
                    names = [d.get("name") for d in datasources]
                    results.append(CheckResult(
                        "Grafana: Prometheus datasource", "FAIL",
                        f"No Prometheus datasource found. All: {names}",
                        "Add Prometheus datasource in Grafana → Connections → Data sources"
                    ))
            else:
                results.append(CheckResult(
                    "Grafana: Prometheus datasource", "WARN",
                    f"HTTP {r.status_code} from /api/datasources"
                ))
        except Exception as e:
            results.append(CheckResult("Grafana: Prometheus datasource", "WARN", str(e)[:80]))

        # 4 — /api/ds/query test WITH uid (your Grafana requires it)
        try:
            now   = datetime.now(timezone.utc)
            start = now - timedelta(minutes=5)

            if ds_uid:
                # Build body with uid — required by your Grafana version
                body = {
                ...
                if r.status_code == 200:
                    ...
                    results.append(CheckResult(
                        "Grafana: PromQL execution (up)", "PASS",
                        f"{len(frames)} frame(s) via /api/ds/query with uid={ds_uid[:8]}..."
                    ))
                else:
                    # Try datasource proxy fallback
                    proxy_url = f"{base_url}/api/datasources/proxy/uid/{ds_uid}/api/v1/query"
                    r2 = await client.get(
                        proxy_url, headers=_hdrs(), params={"query": "up"}
                    )
                    if r2.status_code == 200:
                        count = len(r2.json().get("data", {}).get("result", []))
                        results.append(CheckResult(
                            "Grafana: PromQL via proxy (up)", "PASS",
                            f"{count} series via datasource proxy uid={ds_uid[:8]}..."
                        ))
                    else:
                        results.append(CheckResult(
                            "Grafana: PromQL execution", "FAIL",
                            f"ds/query={r.status_code}  proxy={r2.status_code}: {r2.text[:100]}",
                            "Check token has datasource query permission"
                        ))
            else:
                results.append(CheckResult(
                    "Grafana: PromQL execution", "WARN",
                    "Skipped — no Prometheus datasource UID found in step 3"
                ))
        except Exception as e:
...
async def main():
    ...
    p.add_argument("--grafana-token",  default=os.getenv("GRAFANA_TOKEN", "") or
                                               os.getenv("GRAFANA_SERVICE_ACCOUNT_TOKEN",""),
                   help="Grafana service account token (glsa_...)")
    p.add_argument("--grafana-ds-uid", default=os.getenv("GRAFANA_DS_UID",""),
    ...
    # Set DS UID into env so discovery uses it
    if args.grafana_ds_uid:
        os.environ["GRAFANA_DS_UID"] = args.grafana_ds_uid
    ...
    ok = await run_all_checks(
        ...
        grafana_ds_uid   = args.grafana_ds_uid,
